...
contract BionLock is IBionLock {
    ...
    function _withdrawableTokens(Lock memory userLock) internal view returns (uint256) {
        ...
        uint256 cycleReleaseAmount = (userLock.amount * userLock.cycleBps) / 10_000;
    ...
    function _isValidLpToken(address token, address factory) private view returns (bool) {
        ...
        address factoryPair = IFactory(factory).getPair(pair.token0(), pair.token1());
